...
public class UserController {
    ...
    //Implemeningt a feature wherein the password entered by the user must contain at least 1 alphabet (a-z or A-Z), 1 number (0-9) and 1 special character (any character other than a-z, A-Z and 0-9)
    ...
    public String registerUser(User user, Model model) {
        ...
        if( userPassword.matches("(?=.*[~!@#$%^&*()_-]).*") ){
            iPasswordScore += 1;}
        //If the password satisfies all the conditions, the method returns login
        if (iPasswordScore>=3) {

            return "users/login.html";

        }
        // If the password strength is not, the methods returns to registration with error message.
        else{
